...
pub async fn write_message<T: Serialize>(
    ...
    eyre::ensure!(size <= max_size, "message too large");
...
pub async fn read_message<T: DeserializeOwned>(
    ...
    eyre::ensure!(rest.is_empty(), "cannot decode message: trailing bytes");
...
mod tests {
    ...
    async fn sizes_are_bounded_on_both_sides() {
        ...
        let result: eyre::Result<Vec<u8>> = read_message(&mut server, 256).await;
